...
public final class DeathLocationDao {

    private final Database database;

    public DeathLocationDao(Database database) {
        this.database = database;
    }

    public void save(UUID uuid, Location location) {
        try (PreparedStatement ps = database.connection().prepareStatement(
                "INSERT INTO death_locations (uuid, world, x, y, z, yaw, pitch) VALUES (?, ?, ?, ?, ?, ?, ?) " +
            ...
            throw new IllegalStateException("사망 위치 저장에 실패했습니다.", e);
        }
    }

    public Location load(UUID uuid) {
        try (PreparedStatement ps = database.connection().prepareStatement(
                "SELECT * FROM death_locations WHERE uuid = ?")) {
            ps.setString(1, uuid.toString());
            ...
            throw new IllegalStateException("사망 위치 조회에 실패했습니다.", e);
        }
    }
}
