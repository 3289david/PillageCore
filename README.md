...
전용 월드(`pillage_minigames`)에 지어진 아레나 4종을 관리자 개입 없이 누구나 자유롭게 참가할 수 있습니다.

| 명령어 | 설명 |
|---|---|
| `/minigame join <spleef\|tntrun\|parkour\|tag>` | 해당 미니게임에 참가 |
| `/minigame leave` | 참가 중인 미니게임에서 나가기 (원래 있던 곳/인벤토리로 복귀) |
| `/minigame list` | 네 게임의 현재 상태(대기/카운트다운/진행 중)와 인원 확인 |

- **스플레프**: 눈 블록으로 된 원형 발판에서 삽으로 서로 발밑을 부숴 떨어뜨리는 게임. 최소 2명, 마지막까지 남는 사람이 승리.
- **TNT 런**: 밟은 블록이 약 0.5초 뒤 사라지는 발판 위에서 버티는 게임. 최소 2명, 마지막까지 남는 사람이 승리.
- **파쿠르 레이스**: 정해진 장애물 코스를 가장 먼저 완주하는 게임. 혼자서도 참가해 기록 연습이 가능하고, 떨어지면 처음부터 다시 시작.
...
외형은 일반 약탈자(Ravager)를 기반으로, 몸집을 2.2배로 키우고 항상 붉게 빛나며(스코어보드 팀 글로우 컬러), 발밑에 붉은 입자가 원형으로 계속 회전하고 몸에서 불꽃이 피어오르는 오라 연출이 상시 적용됩니다. 등 위에는 **검은 바탕에 붉은 해골 문양이 그려진 깃발을 든 "약탈 대장"(필리저)이 무적 상태로 탑승**해 있어(실제 레이드의 깃발 든 대장 연출을 그대로 재활용), 일반 약탈자와는 한눈에 구분됩니다. 리소스팩 없이 순정 클라이언트만으로도 보이는 연출입니다.
...
| `/statue` (후원자 전용) | 자기 자신의 얼굴을 한 아머스탠드 동상을 현재 위치에 설치 (10초 쿨다운, 연속 스팸 설치 방지) |
| `/statue remove` (후원자 전용) | 지금 있는 서버(월드)에 내가 설치한 동상 전부 제거 |
| `/statue remove <player>` / `/statue remove all` (관리자) | 다른 사람이 설치한 동상, 또는 지금 서버의 동상 전부를 강제로 제거 |
...
| `/mini admin <on\|off>` | (관리자) 미니서버 **생성**을 서버 전체에서 금지/재허용. 이미 만들어진 미니서버는 금지 중에도 계속 입장 가능 |

- 미니서버는 **누구나 자유롭게** 만들 수 있고, 생성 즉시 그 서버 전용의 새 월드가 만들어져 그 안에서 바로 시작합니다. 관리자가 `/mini admin off`로 막아두면 `/mini create`가 거부됩니다(재시작 없이 즉시 적용, 서버 전체에 적용).
...
- 이 모든 분리는 **네더/엔드에서도 동일하게 적용됩니다** — 미니서버나 허브 안에서 포탈로 네더/엔드에 들어가도 여전히 그 서버 소속으로 인식되어, 인벤토리/엔더상자/홈 등이 엉뚱하게 전체 서버 것으로 새지 않습니다. 미니서버를 삭제하면 그 서버의 네더/엔드 월드도 함께 삭제됩니다.
...
**이미 만들어진 허브 월드/데이터를 명령어로 완전히 지우기**: `config.yml`을 수동으로 고치고 파일을 직접 삭제하는 대신, `/hub delete confirm`(관리자) 한 번으로 처리할 수 있습니다. 허브 안에 있던 플레이어는 전체 서버로 이동되고, 허브 월드와 `hub.db`가 디스크에서 완전히 삭제되며, `config.yml`의 `hub.enabled`도 자동으로 `false`로 저장되어 재시작해도 다시 생기지 않습니다. 명예의 전당이 허브 안에 있었다면, 삭제 직전에 자동으로 전용 월드(`pillage_halloffame`)로 옮겨지므로 후원자 동상이 사라지지 않습니다. 되돌릴 수 없는 작업이라 `confirm`을 붙이지 않으면 경고만 뜨고 아무 일도 일어나지 않습니다.
