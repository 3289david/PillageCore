...
팀 기반 약탈(Raid) PvP 서버를 위한 올인원 Paper 플러그인입니다. 팀, 텔레포트, 물물교환, 레이드, PvP 부가기능, 개인 통계, 서버 관리, QoL, 보상, 에메랄드 경제, 아이템 교환 상점, 채팅 확장, 후원자 등급, GUI 메뉴, 널널한 안티치트에 더해 **친구들끼리 만드는 완전히 독립된 미니서버 시스템**까지 포함되어 있습니다. 레이드/전투 중이라는 이유로 로그아웃 시 처형하는 기능은 없지만, 비행 중이거나(엘리트라 등) 방금 다른 플레이어와 맞붙은 상태에서는 플러그인 텔레포트(`/tpa`, `/back`, `/home` 등)를 막습니다 — 엔더펄/코러스프룻 같은 바닐라 순간이동은 그대로 가능합니다.
...
이벤트 상자는 확률 가중치가 있습니다 — 대부분(약 79%)은 철 주괴 8개/금 주괴 8개/에메랄드 5개/다이아몬드 3개 같은 보상이고, 아주 가끔(항목당 약 2.1%, 합쳐서 약 21%) 인챈트 3~4개짜리 네더라이트 장비/활/석궁/삼지창 같은 OP 아이템이 나옵니다.
...
- **완전히 독립된 세계입니다**: 팀, 잔액, 킬/데스 통계, 홈, 상점 목록, **인벤토리(갑옷·오프핸드 포함)**, **체력·배고픔(포만감 포함)** 이 전체 서버·다른 미니서버와 전혀 공유되지 않습니다 — 새 미니서버는 항상 텅 빈 인벤토리 + 풀피/풀배고픔으로 시작합니다. 서버를 옮기면 그 자리에서 바뀌고, **같은 서버로 다시 돌아오면 떠날 때 그대로** 복원됩니다. (후원자 등급·명예의 전당·차단 기록처럼 "그 사람 자체"에 딸린 정보는 예외로, 어디에서나 동일하게 유지됩니다.)
...
플러그인이 처음 켜질 때 **허브** 월드(스폰이 아니라 별도로 자동 생성되는 로비 월드, `/hub`로 이동)에 기념관이 한 번만 자동으로 지어집니다. 체커보드 무늬 쿼츠 바닥에 낮은 테두리 난간과 화분, 양옆으로 늘어선 쿼츠 기둥(꼭대기마다 랜턴, 밑동마다 불이 타는 캠프파이어)과 처마가 딸린 지붕, 정면 입구부터 이어지는 카펫 통로로 이루어진 개방형 회랑 구조입니다. 후원자 한 명당 받침대 + 자기 얼굴 동상 한 기가 놓이고, 정면에 "명예의 전당" 홀로그램 문구가 떠 있습니다. 처음 접속(또는 있던 서버가 삭제된 뒤 재접속)하면 바로 이 허브의 회랑 입구(정면, 난간이 없는 쪽)로 스폰됩니다.

- 후원자가 추가/제거되면 동상도 자동으로 세워지거나 철거됩니다.
- 동상은 `pillage.admin` 권한자를 제외한 누구도 부술 수 없습니다. **허브 월드 전체(기념관 안팎 어디든)에서 블록 설치/파괴가 관리자 외에는 완전히 금지됩니다** — 관리자는 허브를 자유롭게 꾸미고 고칠 수 있습니다.
- 허브와 명예의 전당은 서버당 한 번만 지어지며, 위치는 DB에 저장되어 재시작해도 같은 자리를 계속 씁니다. 후원자는 전체 서버/미니서버 어디에 있든 동일한 사람이므로, 명예의 전당은 미니서버마다 따로 만들어지지 않고 허브 하나에만 존재합니다.
- 허브는 항상 낮(시간 고정, 낮/밤 순환 없음)이고, 몬스터와 동물을 포함한 모든 자연 스폰이 완전히 차단됩니다(관리자의 `/summon` 등 명령어 스폰은 예외).
...
- **게임플레이 DB** (인스턴스마다 완전히 별도 파일 — `pillage.db`(전체 서버), `hub.db`(허브), 미니서버마다 `instance-<id>.db`): `teams`, `team_members`, `homes`, `last_locations`, `trade_log`, `kill_log`, `tp_log`, `player_stats`, `death_locations`, `daily_rewards`, `playtime_rewards`, `economy`, `shop_offers`, `player_inventory`(인스턴스별 인벤토리 스냅샷), `player_position`(인스턴스별 마지막 위치), `player_vitals`(인스턴스별 체력·배고픔 스냅샷). 미니서버를 만들면 이 테이블들이 전부 빈 상태인 새 파일로 시작합니다.
- **글로벌 DB** (`global.db` 하나, 모든 인스턴스가 공유): `donors`, `donor_pets`, `hall_of_fame`, `hall_of_fame_meta`, `ban_log`, `report_log`, `instances`(미니서버 목록), `player_last_instance`(플레이어별 마지막 접속 서버), `event_box_claims`(플레이어별 미수령 이벤트 상자 개수). 후원자 등급이나 차단 기록처럼 인스턴스를 넘나들어도 유지되어야 하는 데이터입니다.
