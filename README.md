...
| `/eventbox give <player> [수량]` (관리자) | 이벤트 상자(번들 아이템) 지급, 우클릭하면 랜덤 보상 획득 (아래 참고) |
...
플러그인이 처음 켜질 때 스폰(`config.yml`의 `spawn.x`/`spawn.z`) 위치에 작은 기념 광장이 한 번만 자동으로 지어집니다. 매끈한 쿼츠 바닥, 후원자 한 명당 받침대 + 자기 얼굴 동상 한 기, 정면에 "명예의 전당" 홀로그램 문구로 구성됩니다. `/spawn`의 착지 지점은 그 자리의 가장 높은 블록 위로 자동 스냅되므로, 광장이 스폰 좌표에 지어지면 `/spawn`을 치자마자 바로 광장 위로 도착합니다.

- 후원자가 추가/제거되면 동상도 자동으로 세워지거나 철거됩니다.
- 동상과 받침대, 바닥은 (관리자를 제외한) 누구도 부수거나 그 위에 블록을 놓을 수 없습니다.
