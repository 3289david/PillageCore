...
- 명예의 전당은 아래에 설명된 대로 **전체 약탈 서버 안**에 지어집니다.
- 이미 허브가 있는 상태로 운영하다가 나중에 끄면, 그 시점에 허브에 있던 플레이어의 "마지막 서버" 기록은 자동으로 전체 서버로 보정됩니다.

### 명예의 전당

플러그인이 처음 켜질 때 **허브** 월드(스폰이 아니라 별도로 자동 생성되는 로비 월드, `/hub`로 이동)에 기념관이 한 번만 자동으로 지어집니다. `hub.enabled: false`로 운영 중이라면 허브 대신 **전체 약탈 서버**의 (0, 0) 좌표 근처에 똑같이 지어집니다(그 자리에 기존 지형/건축물이 있다면 정리되니, 아직 비어있는 상태에서 켜는 걸 권장합니다). 체커보드 무늬 쿼츠 바닥에 낮은 테두리 난간과 화분, 양옆으로 늘어선 쿼츠 기둥(꼭대기마다 랜턴, 밑동마다 불이 타는 캠프파이어)과 처마가 딸린 지붕, 정면 입구부터 이어지는 카펫 통로로 이루어진 개방형 회랑 구조입니다. 후원자 한 명당 받침대 + 자기 얼굴 동상 한 기가 놓이고, 정면에 "명예의 전당" 홀로그램 문구가 떠 있습니다. 처음 접속(또는 있던 서버가 삭제된 뒤 재접속)하면 바로 이 회랑 입구(정면, 난간이 없는 쪽)로 스폰됩니다.

- 후원자가 추가/제거되면 동상도 자동으로 세워지거나 철거됩니다.
- 동상은 `pillage.admin` 권한자를 제외한 누구도 부술 수 없습니다(어느 월드에 있든 동일). **허브가 있는 경우, 허브 월드 전체(기념관 안팎 어디든)에서 블록 설치/파괴가 관리자 외에는 완전히 금지됩니다** — 관리자는 허브를 자유롭게 꾸미고 고칠 수 있습니다. 이 전체 잠금은 허브에만 적용되며, `hub.enabled: false`라 전체 서버 안에 지어진 경우 그 주변 블록은 평소처럼 일반 규칙(관리자 아니어도 설치/파괴 가능)을 따릅니다.
- 허브와 명예의 전당은 서버당 한 번만 지어지며, 위치는 DB에 저장되어 재시작해도 같은 자리를 계속 씁니다. 후원자는 전체 서버/미니서버 어디에 있든 동일한 사람이므로, 명예의 전당은 미니서버마다 따로 만들어지지 않고 하나만 존재합니다.
- 허브는 항상 낮(시간 고정, 낮/밤 순환 없음)이고, 몬스터와 동물을 포함한 모든 자연 스폰이 완전히 차단됩니다(관리자의 `/summon` 등 명령어 스폰은 예외) — `hub.enabled: false`인 경우 전체 서버는 평소처럼 밤도 오고 몹도 스폰됩니다.
