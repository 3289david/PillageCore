...
- **완전히 독립된 세계입니다**: 팀, 잔액, 킬/데스 통계, 홈, 상점 목록, **인벤토리(갑옷·오프핸드 포함)**, **체력·배고픔(포만감 포함)**, **물약 효과(버프/디버프)** 가 전체 서버·다른 미니서버와 전혀 공유되지 않습니다 — 새 미니서버는 항상 텅 빈 인벤토리 + 풀피/풀배고픔 + 효과 없는 상태로 시작합니다. 서버를 옮기면 그 자리에서 바뀌고, **같은 서버로 다시 돌아오면 떠날 때 그대로** 복원됩니다. (후원자 등급·명예의 전당·차단 기록처럼 "그 사람 자체"에 딸린 정보는 예외로, 어디에서나 동일하게 유지됩니다.)
