...
팀 기반 약탈(Raid) PvP 서버를 위한 올인원 Paper 플러그인입니다. 팀, 텔레포트, 물물교환, 레이드, PvP 부가기능, 개인 통계, 서버 관리, QoL, 보상, 에메랄드 경제, 아이템 교환 상점, 채팅 확장, 후원자 등급, GUI 메뉴, 널널한 안티치트까지 전부 포함되어 있습니다. 레이드/전투 중이라는 이유로 텔레포트를 막거나 로그아웃 시 처형하는 기능은 없지만, 비행 중이거나(엘리트라 등) 방금 다른 플레이어와 맞붙은 상태에서 엔더펄/코러스프룻으로 도주하는 것은 막습니다.
...
모든 텔레포트는 5초 카운트다운 + 이동 시 취소 + 쿨타임이 적용됩니다. 전투 중이거나 소속 팀이 레이드당하는 중이라도 `/tpa`, `/back`, `/home` 등은 그대로 사용할 수 있지만, **비행(엘리트라 글라이딩) 중에는 텔레포트를 사용할 수 없습니다.**
...
| `/eventbox give <player> [수량]` (관리자) | 이벤트 상자 지급, 우클릭하면 랜덤 OP 아이템(네더라이트 장비/토템/엘리트라 등) 획득 |
...
- **전투 중 도주 방지**: 플레이어끼리 서로 때리면 30초간 "전투 중" 상태가 되고, 그 동안에는 엔더펄/코러스프룻으로 순간이동할 수 없습니다(`config.yml`의 `combat.tag-duration-seconds`). 그 외 플러그인 명령어 텔레포트나 로그아웃에는 영향이 없습니다.
...
- `combat.tag-duration-seconds`: 플레이어끼리 서로 때린 뒤 엔더펄/코러스프룻 도주가 막히는 시간(초)
...
`teams`, `team_members`, `homes`, `last_locations`, `trade_log`, `kill_log`, `report_log`, `ban_log`, `tp_log`, `player_stats`, `death_locations`, `daily_rewards`, `playtime_rewards`, `economy`, `shop_offers`, `donors` 테이블로 구성됩니다. 전부 `plugins/PillageCore/pillage.db` 한 파일에 저장됩니다.

## 구현 현황

**전부 완료** — 최초 기획서(팀/TP/거래/약탈/전투/안티치트/랜드(의도적으로 없음)/PvP/GUI/통계/서버관리/QoL/보상/경제/채팅) 16개 대분류에 더해, 비행 중 텔레포트 차단, 전투 중 도주(엔더펄) 차단, OP 아이템 랜덤박스, 후원자 등급(닉네임 그라데이션/배지/입퇴장 연출/파티클/폭죽/동상) 기능을 모두 구현했습니다.
